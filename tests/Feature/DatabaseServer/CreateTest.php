...
test('duplicateBackup clones an entry with a cleared id', function () {
    $user = User::factory()->create();
    $volume = Volume::create(['name' => 'V', 'type' => 'local', 'config' => ['path' => '/x']]);
    ...
    $component = Livewire::actingAs($user)
        ...
        ->set('form.backups.0.retention_policy', 'days')
        ->set('form.backups.0.retention_days', 30)
        ->call('duplicateBackup', 0)
        ->assertCount('form.backups', 2);

    $backups = $component->get('form')->backups;
    expect($backups[1]['id'])->toBeNull()
        ->and($backups[1]['volume_id'])->toBe($volume->id)
        ->and($backups[1]['retention_days'])->toBe(30);
});

test('switching retention_policy to days restores the default retention_days when blank', function () {
    $user = User::factory()->create();
    ...
    Livewire::actingAs($user)
        ...
        ->set('form.database_type', 'mysql')
        ->set('form.backups.0.retention_days', null)
        ->set('form.backups.0.retention_policy', 'forever')
        ->set('form.backups.0.retention_policy', 'days')
        ->assertSet('form.backups.0.retention_days', 14);
});

test('switching retention_policy to gfs restores default tiers when all tiers are blank', function () {
    $user = User::factory()->create();
    ...
    Livewire::actingAs($user)
        ...
        ->set('form.backups.0.retention_policy', 'forever')
        ->set('form.backups.0.retention_policy', 'gfs')
        ->assertSet('form.backups.0.gfs_keep_daily', 7)
        ->assertSet('form.backups.0.gfs_keep_weekly', 4)
        ->assertSet('form.backups.0.gfs_keep_monthly', 12);
});
