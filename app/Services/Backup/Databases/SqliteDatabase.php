...
class SqliteDatabase implements DatabaseInterface
{
     ...
     */
    private function getSftpFilesystem(): ?\League\Flysystem\Filesystem
    {
        $sshConfig = $this->config['ssh_config'] ?? null;
        $sshConfigArray = $this->config['ssh_config_array'] ?? null;

        if ($sshConfig instanceof \App\Models\DatabaseServerSshConfig) {
     ...
     * @return array{success: bool, message: string, details: array<string, mixed>}
     */
    private function testRemotePaths(array $paths, \League\Flysystem\Filesystem $filesystem, string $sshHost): array
    {
        try {
            $failures = [];
            $outputEntries = [];
