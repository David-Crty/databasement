...
class DatabaseServer extends Model
{
    ...
    /**
     * Move type-specific fields (auth_source, srv_enabled, tls_enabled,
     * replica_set, connection_options, dump_flags, ssl_enabled) into extra_config.
    ...
    public static function buildExtraConfig(array &$data, ?array $existingExtraConfig = null, ?string $previousType = null): void
    {
        ...
        $rules = [
            ...
            ['tls_enabled',         fn ($v) => $type === DatabaseType::MONGODB->value && $v,                       fn () => true],
            ['replica_set',         fn ($v) => $type === DatabaseType::MONGODB->value && $v !== '' && $v !== null, fn ($v) => $v],
